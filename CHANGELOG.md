...
- Routine Nginx apply now performs one targeted compilation, one syntax check, and one graceful reload. It no longer runs a full configuration dump, preflights the PID file, or repeatedly polls the process table, keeping healthy apply time independent of site size and within the UI's bounded wait. Exact orphan recovery is exceptional and bounded to the affected port. ([#39](https://github.com/amsive/local-media-proxy/issues/39))
