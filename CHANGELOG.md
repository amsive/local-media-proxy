...
- Nginx activation no longer depends on Local's service identifier matching the runtime process name. Versioned or aliased service records use Nginx's native reload result, and its exact stale-master-PID error triggers one targeted restart of only that site's selected Nginx service during an explicit apply, enable, or disable action. The replacement must then accept one site-scoped reload before settings are saved; a duplicate process still holding the site port now rolls back with full-Local-restart guidance instead of reporting success. Background reconciliation never restarts the service. ([#39](https://github.com/amsive/local-media-proxy/issues/39))
