...
- Nginx activation no longer depends on Local's service identifier matching the runtime process name. Versioned or aliased service records use Nginx's native reload result, and its exact stale-master-PID error triggers one targeted restart of only that site's selected Nginx service during an explicit apply, enable, or disable action. Background reconciliation never restarts the service. ([#39](https://github.com/amsive/local-media-proxy/issues/39))
