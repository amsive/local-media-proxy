...
3. Test the packed artifact with `npm run package:addon` and Local’s **Install from disk** flow.
...
Local cannot overwrite an installed add-on with the same slug. For manual upgrade testing, disable and remove the current Local Media Proxy installation before selecting the replacement archive.
