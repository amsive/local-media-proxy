...
1. Download `local-media-proxy-<version>.tgz` from the matching [GitHub release](https://github.com/amsive/local-media-proxy/releases). Optionally verify it against the attached `.sha256` file.
2. Open Local and go to **Add-ons → Installed**.
3. Choose **Install from disk** and select the `.tgz`.
4. Enable the add-on and restart Local if prompted.

The npm-style `.tgz` is the installable artifact; do not extract it first.
...
Pushing an exact SemVer release tag such as `v0.1.0` starts the release workflow. It verifies that the tag, package metadata, lockfile, changelog, and archive agree before GitHub creates a draft, attaches the installable `.tgz` and SHA-256 checksum, and preselects the prerelease state without marking it Latest. A maintainer reviews the draft but does not publish it manually. The manual **Promote release** workflow requires exact tag-specific confirmation, re-verifies the release, tag, asset identities, and digests, then publishes it as stable and Latest. The documented `release-approval` environment adds a second reviewer gate when configured.
