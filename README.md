...
6. Select **Save & apply** after changing connection setup, then turn on **Enable for this site**. The enable switch saves and applies its on/off state immediately.
...
Nginx and Apache keep separate saved connection profiles, so switching Local's web-server type does not discard either setup. The on/off intent is shared. When the proxy is on and the destination server has a complete saved profile, the add-on automatically reapplies that profile after the switch. If that server still needs setup, activation controls remain unavailable; the Overview information tooltip and Tools help explain what is missing.
...
To disable it, turn off either the **Enable for this site** switch or the compact **Media Proxy** switch on the site's Overview tab. The change saves and applies immediately. Saved connection fields remain available for convenient re-enabling, while all managed Nginx and Apache files are removed and the selected service is refreshed. If Local cannot resolve or load that service, cleanup is deferred without changing settings or files; the Overview information tooltip and Tools help explain the problem so you can stop the site, restore the service, and retry without falsely claiming the compiled proxy is inactive.
