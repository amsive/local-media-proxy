...
Releases are built by GitHub Actions from an exact `v<SemVer>` tag. A read-only build job clean-installs locked dependencies without lifecycle scripts, validates the add-on, builds and inspects the installable ZIP, and preserves the verified assets. A separate write-scoped job creates a GitHub draft, attaches `local-media-proxy-<version>.zip` and `local-media-proxy-<version>.zip.sha256`, and preselects **prerelease** while explicitly leaving **Latest** off. The installer ZIP must contain exactly one top-level folder named `local-media-proxy/`, with no version number in that folder name.
...
9. Run `node scripts/verify-release-package.js v<version> dist/local-media-proxy-<version>.zip`. The verifier enforces the exact asset name, the single versionless `local-media-proxy/` root, and the packaged-file manifest.
...
11. Select that exact ZIP directly in Local's **Install from disk** flow without extracting it, then complete the end-to-end checklist in `CONTRIBUTING.md` against the minimum supported Local version, including the Installed Add-ons card, Overview and Release notes tabs, and both renderer themes.
...
3. Download the attached `.zip`, select it directly in Local without extracting it, and complete the release smoke test. Confirm it contains exactly one top-level `local-media-proxy/` folder.
