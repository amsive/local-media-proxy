...
Lifecycle notifications do not perform awaited filesystem work. They schedule bounded readiness checks that re-read the current site record and status. Background reconciliation verifies saved WP Engine provenance from Local's current site connection without waiting on provider APIs; authoritative provider checks remain part of explicit discovery, connection testing, and settings changes. A stable status alone is not enough: the site root and Local-owned server templates must already exist before any managed path is resolved. A readiness check never creates a directory or template to make the site appear ready.
