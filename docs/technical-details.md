...
After successful compilation and validation, a running Nginx site uses one site-scoped graceful reload. Nginx's native reload result is authoritative; during an explicit apply, enable, or disable action, only its exact stale-master-PID error triggers one targeted restart of that site's selected Nginx service. Background reconciliation reports a reload failure without restarting the service. There is no process polling, retry loop, or dependency on Local's service key matching a process name. Apache performs its own bounded, site-scoped graceful reload using the selected site configuration. Stopped sites are compiled and validated without being started, and no other site's service is refreshed.
