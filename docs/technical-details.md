...
After successful compilation and validation, a running Nginx site uses one site-scoped graceful reload. Nginx's native reload result is authoritative; during an explicit apply, enable, or disable action, only its exact stale-master-PID error triggers one targeted restart of that site's selected Nginx service. The replacement has a sub-second window to accept one site-scoped confirmation reload before the transaction commits. If another process still owns the site's internal Nginx port, the operation rolls back and asks the user to fully quit and reopen Local. Background reconciliation reports a reload failure without restarting the service. There is no process polling, unbounded retry loop, or dependency on Local's service key matching a process name. Apache performs its own bounded, site-scoped graceful reload using the selected site configuration. Stopped sites are compiled and validated without being started, and no other site's service is refreshed.

Local's domain router is a separate global Nginx process that owns ports 80 and 443. The add-on does not inspect, stop, restart, or reconfigure that router. If Local displays its domain port-conflict banner, the conflict must be resolved in Local before any site served through that domain—including otherwise healthy proxied assets—can be reached.
