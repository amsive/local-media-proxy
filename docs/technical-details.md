...
Nginx suppresses incoming request headers before reconstructing only `Host`, the fixed add-on `User-Agent`, `Range`, and `If-Range`. Apache explicitly removes known browser identity, content-negotiation, tracing, credential, cookie, authorization, nonce, CSRF, and proxy-identity headers, then preserves the same fixed identity and range behavior. Future benign header names do not block proxying. Runtime tests verify the resulting upstream header set and compatibility with an unrecognized benign browser header.
...
Lifecycle notifications do not perform awaited filesystem work. They schedule bounded readiness checks that re-read the current site record and status. Background reconciliation verifies saved WP Engine provenance from Local's current site connection without waiting on provider APIs. Explicit discovery, connection testing, and changed settings still perform authoritative remote checks; an unchanged previously verified profile reuses its saved verification while retaining the same local provenance checks. A stable status alone is not enough: the site root and Local-owned server templates must already exist before any managed path is resolved. A readiness check never creates a directory or template to make the site appear ready.
...
Any queued transaction revalidates the current site, status, primitive service identity, and managed paths immediately before each write, atomic rename, or unlink. It does not recursively serialize Local's service metadata. Rollback is similarly qualified at every restoration step and proceeds only while the current site remains lifecycle-ready. If the site is deleting or no longer exists, the transaction stops without restoring settings, recreating directories, refreshing a service, or reading the removed root.
...
After successful compilation and validation, a running Nginx site uses one site-scoped graceful reload. Nginx's native reload result is authoritative; its exact stale-master-PID error can still trigger one targeted restart and confirmation reload. Before a running-site action mutates proxy state, the add-on also checks Local's own tracked service state. Healthy services take the immediate path without a process scan. On macOS, when Local has lost the selected Nginx or Apache process, the add-on first requires Local's existing exact selected-service process to be safely restartable, then inspects only the site's exact frontend listener. It accepts only a current-user process tree rooted at PPID 1 whose executables exactly match Local's known Nginx or Apache binaries, rechecks the same PID, parent, and user records, sends only `SIGTERM`, and waits at most two seconds for that listener to clear. This supports an orphaned sibling web server after switching between Nginx and Apache. The captured selected-service process is restarted directly and must remain tracked before any managed files or settings change. An unowned port can receive the same targeted restart; unrelated or mixed ownership fails without a signal or mutation. The same exact check runs inside the exceptional Nginx stale-master recovery path before its targeted restart. Enabled startup reconciliation performs this repair only after three stable one-second lifecycle samples. Stopped sites are compiled without process recovery or restart, and PHP-FPM is outside this feature.

Local's domain router is a separate global Nginx process that owns ports 80 and 443. A healthy tracked router is accepted immediately and is not scanned or restarted. On macOS, a running configured site in standard domain-routing mode can enter the exceptional recovery path only after Local's existing router process has settled in a restartable failed state and every Local site is running or stopped. The add-on checks exact ownership on both ports using only Local's selected router Nginx binary; a verified orphan receives `SIGTERM`, after which the captured Nginx process is restarted directly and both listeners are verified against that captured process tree. It does not call Local's global router refresh, rebuild route or certificate files, update hosts, or iterate other sites. Site creation, pull, push, and other transitions remain entirely under Local's control. An unowned port can be restarted, but an active, unrelated, or mixed owner remains untouched. The add-on never changes routing mode, domains, site ports, or PHP services.
...
- **Configuration refresh fails:** Review Local's main log and the selected site's Nginx or Apache error log from the same attempt. Resolve Local's domain port-conflict banner separately when present. Failed apply operations restore their previous snapshots only while the site remains lifecycle-ready; a Local transition stops rollback safely.
