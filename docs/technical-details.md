...
After successful compilation and validation, a running Nginx site uses one site-scoped graceful reload. The add-on verifies the contained PID file and the live master's command identity where the platform exposes it, then lets Nginx complete the validated reload without invoking Local's hard service restart or depending on Local's service key as a process name. Nginx for Windows uses its PID-scoped named control event, so an unrelated reused PID cannot receive the reload signal. Apache performs its own bounded, site-scoped graceful reload using the selected site configuration. Stopped sites are compiled and validated without being started, and no other site's service is refreshed.
