...
- Versioned or aliased Local service records now confirm the selected Nginx or Apache process actually started before a running-site refresh. An already-failed service leaves settings and files untouched instead of spawning another process against an occupied internal port, and its error identifies the exact Local and site log files to share. Nginx applies with one targeted compile, syntax check, and graceful reload; only its exact stale-master-PID error after that check permits one targeted restart, whose replacement must accept a site-scoped reload before settings are saved. Apache retains its validated graceful reload and accepts benign future browser-header names while stripping known sensitive headers. Local's separate domain router is never stopped or reconfigured by Media Proxy. Background reconciliation does not restart either site service. ([#32](https://github.com/amsive/local-media-proxy/issues/32), [#39](https://github.com/amsive/local-media-proxy/issues/39))
