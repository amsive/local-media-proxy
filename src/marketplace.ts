...
- Managed source and compiled server configuration converge after settings changes, same-value repair requests, server changes, and confirmed startup drift; passive reads, clean disabled profiles, and already-converged enabled profiles do not reload Nginx. ([#34](https://github.com/amsive/local-media-proxy/issues/34), [#38](https://github.com/amsive/local-media-proxy/issues/38))
- Nginx activation accepts versioned or aliased Local service records and applies with one targeted compile, one syntax check, and one graceful reload instead of a full configuration dump and repeated process polling. ([#39](https://github.com/amsive/local-media-proxy/issues/39))
