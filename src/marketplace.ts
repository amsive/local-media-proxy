...
- Nginx activation accepts versioned or aliased Local service records and applies with one targeted compile, one syntax check, and one graceful reload. Only Nginx's exact stale-master-PID error during an explicit settings action triggers one targeted service restart; background reconciliation does not restart it, and there is no full configuration dump, PID preflight, or process polling. ([#39](https://github.com/amsive/local-media-proxy/issues/39))
