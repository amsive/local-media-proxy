...
- Managed source and compiled server configuration converge after settings changes, same-value repair requests, server changes, and confirmed startup drift; passive reads, clean disabled profiles, and already-converged enabled profiles do not reload Nginx. Background checks use local provenance and primitive service identity without waiting on provider APIs or traversing complete service metadata. ([#34](https://github.com/amsive/local-media-proxy/issues/34), [#38](https://github.com/amsive/local-media-proxy/issues/38))
- Nginx activation accepts versioned or aliased Local service records and applies with one targeted compile, one syntax check, and one graceful reload. Only Nginx's exact stale-master-PID error during an explicit settings action triggers one targeted service restart, and the replacement must accept one site-scoped reload before settings are saved. A duplicate process still holding the site port now rolls back with full-Local-restart guidance instead of reporting success. Background reconciliation does not restart it, and there is no full configuration dump, PID preflight, or process polling. ([#39](https://github.com/amsive/local-media-proxy/issues/39))
