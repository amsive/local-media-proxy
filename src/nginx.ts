...
export function buildManagedNginxConfig(
	...
	return [
		...
		`location ~* ${quoteNginx(UPLOAD_ASSET_URI_PATTERN)} {`,
		'\tif ($request_method !~ ^(GET|HEAD)$) { return 405; }',
		'\tif ($http_transfer_encoding != "") { return 400; }',
		'\tif ($http_content_length !~ ^(?:|0)$) { return 400; }',
		`\tif ($request_uri ~* ${quoteNginx(`^/wp-content/uploads/[^?]*${UNSAFE_RAW_PERCENT_ENCODING_PATTERN}`)}) { return 400; }`,
		`\tif ($request_uri ~* ${quoteNginx('^/wp-content/uploads/(?:[^?]*/)?(?:\\.|%2e){1,2}(?:/|\\?|$)')}) { return 400; }`,
		`\tif ($request_uri ~ ${quoteNginx('^/wp-content/uploads/(?:/|[^?]*//)')}) { return 400; }`,
		...
		`\tif ($uri ~* ${quoteNginx(BLOCKED_UPLOAD_ASSET_PATH_PATTERN)}) { return 404; }`,
